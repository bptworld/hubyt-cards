...
def _render_scroll(label, headline):
    ...
    frames = []
    for step in range(0, 64 + text_w + 16, 2):
        image = Image.new("RGB", (64, 32), (0, 5, 12))
        draw = ImageDraw.Draw(image)
        draw.rectangle((0, 0, 63, 8), fill=(5, 18, 25))
        draw_sharp_text(image, (1, -3), label[:10].upper(), (80, 220, 170), bold)
        draw_sharp_text(image, (64 - step, 14), text, (245, 250, 255), font)
        frames.append(image)
    out = BytesIO()
...
def render(options=None):
    ...
    return _render_scroll(label, items[0]["title"])
