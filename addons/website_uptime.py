...
def render(options=None):
    ...
    draw_sharp_text(image, (1, -3), label[:10].upper(), (80, 180, 255), bold)
    status = "UP" if data["ok"] else "DOWN"
    sw = draw.textbbox((0, 0), status, font=big)[2]
    draw_sharp_text(image, ((64 - sw) // 2, 5), status, color, big)
    ms = f"{data['ms']}ms" if data["ok"] else "NO REPLY"
    mw = draw.textbbox((0, 0), ms, font=font)[2]
    draw_sharp_text(image, ((64 - mw) // 2, 23), ms, (150, 170, 185), font)
    out = BytesIO()
