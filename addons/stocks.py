...
def _draw_ticker_frame(x, items, widths, font, bold):
    ...
    for item, width in zip(items, widths):
        if cursor < 64 and cursor + width > -1:
            _draw_quote_panel(image, cursor, item, font, bold)
        cursor += width
    return image


def _render_ticker(symbols, dwell, scrolls=1):
    ...
    for index in range(frame_count):
        ...
        x = 64 - pass_index * step
        frames.append(_draw_ticker_frame(x, items, widths, font, bold))
...
def render(options=None):
    ...
    try:
        body = _render_ticker(symbols, dwell, scrolls)
